...
const PendingRequests = ({ token }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://sociolib-api.herokuapp.com/friends/requests/`, {
  ...
  const handleDecline = (id) => {
    axios
      .delete(`https://sociolib-api.herokuapp.com/friends/req/${id}/`, {
        headers: { Authorization: `Token ${token}` },
      })
      .then((res) => {
        console.log(res);
  ...
  return (
    <>
      <div className="h-1/2">
        <Heading2 className="text-gray-700 pb-8 pl-16">Requests Received</Heading2>
        <div>
          <ul className="grid gap-10 row-gap-8 mx-auto sm:row-gap-10 lg:max-w-screen-lg sm:grid-cols-2 lg:grid-cols-3">
            {received.map((a) => (
              <li key={a.id}>
                ...
                <div className="flex">
                  <div
                    className="cursor-pointer flex justify-center items-center py-2 pr-1"
      ...
      <div className="h-1/3">
        <Heading2 className="text-gray-700 pb-8 pl-16">Requests Sent</Heading2>
        <div>
          <ul className="grid gap-10 row-gap-8 mx-auto sm:row-gap-10 lg:max-w-screen-lg sm:grid-cols-2 lg:grid-cols-3">
            {sent.map((a) => (
              <li key={a.id}>
